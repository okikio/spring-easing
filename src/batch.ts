...
import { limit, toFixed, scale, isNumberLike, getUnit } from "./utils.ts";
...
*/
export function batchInterpolateNumber(arr_t: number[], values: number[], decimal = 3) {
  // nth index
...
*/
export function batchInterpolateSequence<T>(
  arr_t: number[],
  values: T[]
) {
  // nth index
 ...
 */
export const batchInterpolateUsingIndex = batchInterpolateSequence;
...
*/
export function batchInterpolateString(
  arr_t: number[],
...
*/
export function batchInterpolateComplex<T>(
  arr_t: number[],
  values: T[],
  decimal = 3
) {
  ...
  // Interpolate numbers
  if (isNumber) return batchInterpolateNumber(arr_t, values as number[], decimal);

  // Interpolate strings with numbers, e.g. "5px"
  if (isLikeNumber)
    return batchInterpolateString(arr_t, values as (number | string)[], decimal);

  // Interpolate pure strings and/or other type of values, e.g. "inherit", "solid", etc...
  return batchInterpolateSequence<T>(arr_t, values);
}
 ...
 * ```
 */
export function BatchSpringEasing<T extends unknown[] = number[], TReturn extends unknown[] = T>(
  values: T,
  options: TypeEasingOptions | TypeEasingOptions["easing"] = {},
  customInterpolate: IGenericBatchInterpolationFunction<T, TReturn> = batchInterpolateComplex as unknown as IGenericBatchInterpolationFunction<T, TReturn>
) {
...
export function toAnimationFrames(customInterpolate: TypeInterpolationFunction) {
  return <T extends unknown[] = number[], TReturn extends unknown[] = T>(arr_t: number[], values: T, decimal?: number) => {
    return arr_t.map(t => customInterpolate(t, values, decimal)) as TReturn;
  }
}